...
nmdfile = [auxprefix '/' 'ind_gene_NMD.txt'];
% a list of types (as opposed to imputed) SNPs
typedfile = [auxprefix '/' 'GTd.snps.txt'];
...
knownfile = [auxprefix '/' 'known_ana_geneimprint_su.txt'];
...
ensg2namefile = [auxprefix '/' 'new_ensg2name.txt'];
...
% ph: 1 is het, -1 or 0 is hom
% load([dprefix '/' datafile '.mat'],'ref','n');
n = h5read([dprefix '/' datafile '.mat'], '/n');
n(n < 8) = NaN;
n = double(n);
ref = h5read([dprefix '/' datafile '.mat'], '/ref');
ref(ref < 0) = NaN;
ref = double(ref);
ph = n>0;
ph(ph==-1) = 0;
...
sgen = h5read([dprefix '/' datafile '.mat'], '/sgen');
...
disp('compute per site likeliehood');
...
disp('keep only one snp per gene');
...
disp('compute per-gene and per-gene, per-individual Z');
...
[fid message] = fopen([rprefix '/' datafile '.impglr.res.txt'],'w');
if fid < 0
