#' Generate Sample- and Gene-level ASE data
#'
#' This module processes phASER Gene AE results to generate
...
#' analyze phASER outputs.
#'
#' Terminology:
#' - COV: Genes with total haplotypic read coverage ≥ threshold (default: 20).
#' - ASE: Subset of COV genes filtered by blacklist exclusion,
#'   haplotype ratio ≥ threshold (default: 1.5), and binomial test
#'   with FDR < threshold (default: 0.05).
#' - MAE: COV genes with all reads mapping to one haplotype
#'   (aCount = 0 or bCount = 0).
#'
#' Outputs:
#' - Per-sample:
...
arrange_contig_order <- function(df) {
    #' Rearrange Gene AE file by chromosome
    #'
    #' This function reorders a Gene AE dataframe by genomic contigs
    ...
    #' @examples
    #' \dontrun{
    #' arranged_df <- arrange_contig_order(gene_ae_df)
    #' }
    #'
    #' @export
    df %>%
...
# Parse options
option_list <- list(
    make_option(c("-i", "--input"), type="character", default="../data/output/phaser/",
    ...
    make_option(c("-g", "--gene-coords"), type="character",
                default='../resource/gencode.v.45.gene_coordinates.gene_symbol.bed',
                help="Gene coordinates BED file [default %default]", metavar="character"),
    make_option(c("-c", "--cov_genes-threshold"), type="numeric", default=20,
                help="Coverage threshold [default %default]", metavar="numeric"),
    make_option(c("-r", "--hap-ratio"), type="numeric", default=1.5,
                help="Haplotypic ratio [default %default]", metavar="numeric"),
    make_option(c("-f", "--binom-fdr"), type="numeric", default=0.05,
                help="Binomial FDR [default %default]", metavar="numeric")
)
...
input_dir <- opt$input
out_dir <- opt$output
gene_coordinates_path <- opt$`gene-coords`
cov_threshold <- opt$`cov_genes-threshold`
haplotypic_ratio <- opt$`hap-ratio`
binomial_fdr <- opt$`binom-fdr`
...
# eg. gencode.v.45.gene_coordinates.gene_symbol.bed
gene_coordinates<-read.table(gene_coordinates_path, sep = "\t") 
names(gene_coordinates) <- c("chr", "start", "stop", "name", "gene_symbol")

subdir_list <- list.dirs(path = input_dir, full.names = TRUE, recursive = FALSE)
...
for (subdir in subdir_list) {
    ...
    message(paste("Loading", sample, "phASER Gene AE data..."))
    file_path <- list.files(subdir, pattern = "\\.gene_ae\\.txt$", full.names = TRUE, recursive = TRUE)

    if (length(file_path) == 0) {
      stop("No file ending with '.gene_ae.txt' found in ", subdir)
    } else if (length(file_path) > 1) {
      stop("Multiple '.gene_ae.txt' files found in ", subdir, 
           ": ", paste(basename(file_path), collapse = ", "))
    }

    file_path <- file_path[1]
    gene_ae <- read.delim(file_path)
    
    num_gene_ae<-nrow(gene_ae)
    message(paste0("\t", num_gene_ae, " genes found in Gene AE file."))

    ### COV_N genes ###
    cov_genes <-subset(gene_ae, gene_ae$totalCount >= cov_threshold)
    
    num_cov<-nrow(cov_genes)
    message(paste0("\t", num_cov, " genes with RNA coverage >=", cov_threshold))
    
    # Summed total counts of COV_N genes
    num_totalCount_cov<-sum(cov_genes$totalCount)
    
    # Summed total variants of COV_N genes
    num_totalvar_cov<-sum(cov_genes$n_variants)
    
    cov_genes$binom_p <- apply(cov_genes[, c("aCount", "bCount")], 1, function(counts) {
        success <- counts[1]
        trials <- sum(counts)
        p_value <- binom.test(success, trials, p = 0.5)$p.value
        return(p_value)
    })
    cov_genes$binom_q <- p.adjust(cov_genes$binom_p, method = "fdr")
    
    # Add gene symbol
    cov_genes <- merge(cov_genes, 
                       gene_coordinates[, c("name", "gene_symbol")], by = "name", all.x = TRUE) 

    col_order <- c(
    "contig", "start", "stop", "name", "gene_symbol",
    "aCount", "bCount", "totalCount", "log2_aFC", "n_variants", "variants",
    "binom_p", "binom_q", "gw_phased", "bam"
    )
    cov_reorder <- arrange_contig_order(cov_genes[, col_order])
    ...
    ### ASE genes ###
    # ASE gene:
    #   - Haplotypic ratio >= haplotypic_ratio 
    #   - binomial FDR < binomial_fdr
    ase<-subset(cov_reorder, cov_reorder$binom_q<binomial_fdr)
    ase<-subset(ase, abs(log2_aFC) >= log2(haplotypic_ratio))
    
    # Summed total counts of ASE genes
    num_totalCount_ase<-sum(ase$totalCount)
    ...
    stat <- data.frame(
        ...
        prop_cov = num_cov/num_gene_ae,
        prop_ase = num_ase/num_gene_ae,
        prop_ase_cov = num_ase/num_cov,
        prop_mae = num_mae/num_gene_ae,
